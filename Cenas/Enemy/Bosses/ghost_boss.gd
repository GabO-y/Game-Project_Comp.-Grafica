...
func create_ghosts(pos: Vector2, type: int):
...
func get_random_point_in_segs() -> Vector2:
	var seg: Line2D = segs_to_ghost_run.get_children().pick_random() as Line2D
	var x1 = seg.points[0].x
	var y1 = seg.points[0].y
	var x2 = seg.points[1].x
	var y2 = seg.points[1].y
...
func create_ghosts_and_run():
	...
	for i in range(quant_ghost_create):
		create_ghosts(get_random_point_in_segs(), type_run)
...
func finish_ghosts_run():
	...
	body.global_position = get_random_point_in_segs()
